...
class App extends Component {
  ...
  render() {
    ...
    const displayInlineStyle = { display: "inline-flex" };
    const { todos, current_filter, todo } = this.state;

    let items = todos;
    if (current_filter.length > 0) {
      items = todos.filter(({ cat }) => cat === current_filter);
    }
    console.log("runnibng");
    return (
        ...
        */}
        ...
        >
          <UselessTextInput
            multiline
            numberOfLines={4}
            onChangeText={todo => this.onChangeText(todo)}
            value={todo}
          />
        </View>
        <Button onPress={() => this.onAddButtonClick()} title="Add Todo" />
      </View>
    );
  }
}
